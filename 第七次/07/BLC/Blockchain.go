...
// 数据库名字
const LHJ_dbName = "blockchain_%s.db"

// 表的名字
const LHJ_blockTableName = "blocks"
...
//1. 创建带有创世区块的区块链
func LHJ_CreateBlockchainWithGenesisBlock(address string,nodeID string) *LHJ_Blockchain {

	// 格式化数据库名字
	dbName := fmt.Sprintf(LHJ_dbName,nodeID)
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		if b != nil {
			...
			txCoinbase := LHJ_NewCoinbaseTransaction(address)
...
// 返回Blockchain对象
func BlockchainObject(nodeID string) *LHJ_Blockchain {
...
func (blockchain *LHJ_Blockchain) UnUTXOs(address string,txs []*LHJ_Transaction) []*UTXO {
	...
	for _,tx := range txs {
		...
		for index,out := range tx.LHJ_Vouts {
			...
			if out.LHJ_UnLockScriptPubKeyWithAddress(address) {
...
func (blockchain *LHJ_Blockchain) LHJ_FindSpendableUTXOS(from string, amount int,txs []*LHJ_Transaction) (int64, map[string][]int) {
	...
	for _, utxo := range utxos {
		...
		value = value + utxo.Output.LHJ_Value
...
// 挖掘新的区块
func (blockchain *LHJ_Blockchain) LHJ_MineNewBlock(from []string, to []string, amount []string,nodeID string) {
	...
	//奖励
	tx := LHJ_NewCoinbaseTransaction(from[0])
	txs = append(txs,tx)
...
func (blockchain *LHJ_Blockchain) LHJ_GetBalance(address string) int64 {
	...
	utxos := blockchain.UnUTXOs(address,[]*LHJ_Transaction{})

	var amount int64

	for _, utxo := range utxos {

		amount = amount + utxo.Output.LHJ_Value
	}

	return amount
}

func (bclockchain *LHJ_Blockchain) SignTransaction(tx *LHJ_Transaction,privKey ecdsa.PrivateKey,txs []*LHJ_Transaction)  {
	...
	for _, vin := range tx.LHJ_Vins {
		prevTX, err := bclockchain.LHJ_FindTransaction(vin.LHJ_TxHash,txs)
		if err != nil {
			log.Panic(err)
		}
...
func (bc *LHJ_Blockchain) LHJ_VerifyTransaction(tx *LHJ_Transaction,txs []*LHJ_Transaction) bool {
	...
	for _, vin := range tx.LHJ_Vins {
		prevTX, err := bc.LHJ_FindTransaction(vin.LHJ_TxHash,txs)
		if err != nil {
			log.Panic(err)
		}
		prevTXs[hex.EncodeToString(prevTX.LHJ_TxHash)] = prevTX
	...
	return tx.LHJ_Verify(prevTXs)
}


// [string]*TXOutputs
func (blc *LHJ_Blockchain) LHJ_FindUTXOMap() map[string]*LHJ_TXOutputs  {

	blcIterator := blc.Iterator()

	// 存储已花费的UTXO的信息
	spentableUTXOsMap := make(map[string][]*LHJ_TXInput)


	utxoMaps := make(map[string]*LHJ_TXOutputs)
...
func (bc *LHJ_Blockchain) LHJ_GetBestHeight() int64 {

	block := bc.Iterator().Next()

	return block.LHJ_Height
}

func (bc *LHJ_Blockchain) LHJ_GetBlockHashes() [][]byte {
	...
	for {
		...
		blockHashs = append(blockHashs,block.LHJ_Hash)
