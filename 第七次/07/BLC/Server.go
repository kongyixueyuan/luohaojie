...
func startServer(nodeID string,minerAdd string)  {
	...
	ln,err := net.Listen(LHJ_PROTOCOL,nodeAddress)
...
func handleConnection(conn net.Conn,bc *LHJ_Blockchain) {
	// 读取客户端发送过来的所有的数据
	...
	fmt.Printf("Receive a Message:%s\n",request[:LHJ_COMMANDLENGTH])

	command := bytesToCommand(request[:LHJ_COMMANDLENGTH])

	switch command {
		case LHJ_COMMAND_VERSION:
			handleVersion(request, bc)
		case LHJ_COMMAND_ADDR:
			handleAddr(request, bc)
		case LHJ_COMMAND_BLOCK:
			handleBlock(request, bc)
		case LHJ_COMMAND_GETBLOCKS:
			handleGetblocks(request, bc)
		case LHJ_COMMAND_GETDATA:
			handleGetData(request, bc)
		case LHJ_COMMAND_INV:
