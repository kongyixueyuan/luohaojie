...
func (cli *CLI) Run()  {
	...
	if sendBlockCmd.Parsed() {
		...
		for index,fromAdress := range from {
			if LHJ_IsValidForAdress([]byte(fromAdress)) == false || LHJ_IsValidForAdress([]byte(to[index])) == false {
				fmt.Printf("地址无效......")
				printUsage()
				os.Exit(1)
			}
	...
	if createWalletCmd.Parsed() {
		// 创建钱包
		cli.LHJ_createWallet(nodeID)
	}

	if createBlockchainCmd.Parsed() {

		if LHJ_IsValidForAdress([]byte(*flagCreateBlockchainWithAddress)) == false {
			fmt.Println("地址无效....")
			printUsage()
			os.Exit(1)
		}
	...
	if getbalanceCmd.Parsed() {
		...
		if LHJ_IsValidForAdress([]byte(*getbalanceWithAdress)) == false {
			fmt.Println("地址无效....")
			printUsage()
			os.Exit(1)
		}
